...
public extension Vec3 {
    ...
    init(phi:Double, theta:Double) {
        ...
        self.x = stheta*cos(phi)
        self.y = stheta*sin(phi)
        self.z = ctheta
    }
    var infloat3:SIMD3<Float>  {
        return SIMD3<Float>(self)
    }
    func lerp(vector v:Vec3,coef c:Double) -> Vec3 {
        return self + (v - self) * c
    }
    func clamp(min vmin:Vec3,max vmax:Vec3) -> Vec3 {
        return simd.max(simd.min(self,vmax),vmin)
...
public extension Vec4 {
    ...
    var infloat4:SIMD4<Float>  {
        return SIMD4<Float>(self)
    }
    func lerp(vector v:Vec4,coef c:Double) -> Vec4 {
        return self + (v - self) * c
    }
    var normalized:Vec4 {
        return normalize(self)
...
public struct Vec4 : CustomStringConvertible,JsonConvertible {
    ...
    public var indouble4 : SIMD4<Double> {
