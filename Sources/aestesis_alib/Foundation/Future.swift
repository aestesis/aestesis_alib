...
public class Future: Atom, @unchecked Sendable {
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////////////
    //////////////////////////////////////////////////////////////////////////////////////////////////////////
    public func then(_ fn: @escaping (Future) -> Void) {
        let _ = _done.always(fn)
    }
    public func pulse(_ fn: @escaping (Future) -> Void) {
    ...
    public func error(_ error: Swift.Error, _ f: String = #file, _ l: Int = #line) {
        if state == .inProgress {
            self.result = AlibError(error, f, l)
            state = .error
            _done.dispatch(self)
            if autodetach {
                self.detach()
    ...
    public func onCancel(_ fn: @escaping (Future) -> Void) {
    ...
    #endif
    public init(
        context: String? = nil, file: String = #file, line: Int = #line,
...
public class Job: Future, @unchecked Sendable {
    ...
    public init(
        owner: Node, priority: Priority = Priority.normal, info: String = "",
...
public class Worker: NodeUI, @unchecked Sendable {
    ...
    public func stop() {
        ...
        while threads > 0 {
            ...
            if ß.time - t > 1 {
                Debug.error(
                    "\(threads) thread(s) in worker \(debugName) take too long to exit, stop waiting...",
                    #file, #line)
                #if DEBUG
                    debugInfo()
                #endif
    ...
    public func run(
        _ owner: NodeUI, priority: Job.Priority = Job.Priority.normal, info: String = "",
        action: @escaping () -> Void
    ) -> Job {
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////////////////
public class AlibError: Atom, @unchecked Sendable, Swift.Error {
    public let message: String
    public let file: String
    public let line: Int
    public private(set) var origin: Swift.Error?
    ...
    public init(_ error: AlibError, _ file: String = #file, _ line: Int = #line) {
        self.origin = error
    ...
    public func get<T>() -> T? {
        ...
        if let s: AlibError = self.origin as? AlibError {
            let v: T? = s.get()
            return v
        }
        return nil
