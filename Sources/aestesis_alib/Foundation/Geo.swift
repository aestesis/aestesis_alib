...
public struct GeoPoint {
    ...
    public var location : CLLocation {
        return CLLocation(coordinate:self.locationCoordinate,altitude:0,horizontalAccuracy: 0,verticalAccuracy: 0,course: 0,speed: 0,timestamp: Date())
    }
    ...
    public static func fromGoogle(apiKey:String,address:String,fn: @escaping(GeoPoint?)->()) {
        ...
        let task = URLSession.shared.dataTask(with: components.url!) { data, response, error in
            ...
            do {
                ...
                if let g = GeoPoint(json:json["results"][0]["geometry"]["location"].dictionaryValue) {
...
public struct GeoRect {
    ...
    public func contains(_ point:GeoPoint) -> Bool {
        return point.lat>=southWest.lat && point.lng>=southWest.lng && point.lat<=northEast.lat && point.lng <= northEast.lng
    }
    public func contains(_ rect:GeoRect) -> Bool {
        return rect.southWest.lat>=self.southWest.lat && rect.southWest.lng>=self.southWest.lng && rect.northEast.lat<=self.northEast.lat && rect.northEast.lng<=self.northEast.lng
    }
    public var geojson : JSON {
        return JSON(["type":"Polygon","coordinates":[[[sw.lng,sw.lng],[sw.lng,ne.lat],[ne.lng,ne.lat],[ne.lng,sw.lat]]]] as [String:Any])
    }
...
public func +(lhs: GeoRect, rhs: GeoRect) -> GeoRect {
    return GeoRect(sw:GeoPoint(lng:min(lhs.sw.lng,rhs.sw.lng),lat:min(lhs.sw.lat,rhs.sw.lat)),ne:GeoPoint(lng:max(lhs.ne.lng,rhs.ne.lng),lat:max(lhs.ne.lat,rhs.ne.lat)))
}
public func +(lhs: GeoRect, rhs: GeoPoint) -> GeoRect {
    return GeoRect(w:min(lhs.sw.lng,rhs.lng),s:min(lhs.sw.lat,rhs.lat),e:max(lhs.ne.lng,rhs.lng),n:max(lhs.ne.lat,rhs.lat))
}
/*
