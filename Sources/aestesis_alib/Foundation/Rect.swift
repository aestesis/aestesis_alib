...
public struct Rect: CustomStringConvertible, JsonConvertible {
    ...
    public func intersect(_ r: Rect) -> Bool {
        return
            !(self.right < r.left || self.bottom < r.top || self.left > r.right
            || self.top > r.bottom)
    }
    ...
    public func extend(x: Double = 0, y: Double = 0, w: Double = 0, h: Double = 0) -> Rect {
        var r = self
        r.x -= x
        r.w += x + w
        r.y -= y
    ...
    public func contains(_ rect: Rect) -> Bool {
        return rect.left >= self.left && rect.right <= self.right && rect.top >= self.top
    ...
    public func percent(_ px: Double = 0, _ py: Double = 0, _ pw: Double = 1, _ ph: Double = 1)
        -> Rect
    {
    ...
    public func set(x: Double? = nil, y: Double? = nil, w: Double? = nil, h: Double? = nil) -> Rect
    {
        return Rect(x: x ?? self.x, y: y ?? self.y, w: w ?? self.w, h: h ?? self.h)
    }
    ...
    public func scale(_ scale: Double) -> Rect {
        ...
        return Rect(
            origin: origin.translate(-0.5 * (sz.width - width), -0.5 * (sz.height - height)),
            size: sz)
    }
    ...
    public var strip5: [Point] {
        let c = (topLeft + bottomLeft + topRight + bottomRight) * 0.25
        return [bottomLeft, c, topLeft, c, topRight, c, bottomRight, c, bottomLeft]
    }
    ...
    public init(center: Point, size: Size) {
        self.origin = center - size.point * 0.5
        self.size = size
    }
    public init(center: Point, diagonal: Double, ratio: Double) {
        let size = Size(diagonal: diagonal, ratio: ratio)
        self.origin = center - size.point * 0.5
        self.size = size
    }
...
public func + (lhs: Rect, rhs: Rect) -> Rect {
    return Rect(
        left: min(lhs.left, rhs.left), top: min(lhs.top, rhs.top), right: max(lhs.right, rhs.right),
        bottom: max(lhs.bottom, rhs.bottom))
}
public func + (lhs: Rect, rhs: Point) -> Rect {
    return Rect(
        left: min(lhs.left, rhs.x), top: min(lhs.top, rhs.y), right: max(lhs.right, rhs.x),
        bottom: max(lhs.bottom, rhs.y))
}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////////////////
public struct ColorRect: Equatable {
    public init(
        topLeft: Color, topRight: Color, bottomRight: Color, bottomLeft: Color, midLevel: Double = 1
    ) {
    ...
    public var midLevel: Double

    public static func == (lhs: ColorRect, rhs: ColorRect) -> Bool {
        return lhs.topLeft == rhs.topLeft && lhs.topRight == rhs.topRight
            && lhs.bottomLeft == rhs.bottomLeft && lhs.bottomRight == rhs.bottomRight
    }
    public var strip5: [Color] {
        let c = ((topLeft + bottomLeft + topRight + bottomRight) * 0.25 * midLevel).saturated
        return [bottomLeft, c, topLeft, c, topRight, c, bottomRight, c, bottomLeft]
    }
}
//////////////////////////////////////////////////////////////////////////////////////////////////////////
