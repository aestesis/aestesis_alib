...
public class Filter: NodeUI, @unchecked Sendable {
    let kernel: MPSUnaryImageKernel
    public func process(
        source: Texture2D, destination: Texture2D, _ fn: @escaping @Sendable () -> Void
    ) {
        let cb = viewport!.gpu.queue.makeCommandBuffer()
        kernel.encode(
            commandBuffer: cb!, sourceTexture: source.texture!,
