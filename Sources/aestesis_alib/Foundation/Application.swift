...
public class Application {
    ...
    public func secureUrl(string: String, write: Bool = false) -> Foundation.URL? {
        ...
        #if os(iOS)
            do {
                let scope: Foundation.URL.BookmarkCreationOptions =
                    write
                    ? [.withSecurityScope]
    ...
    public static func localPath(_ path: String) -> String {
        #if os(OSX)
            let sup = NSSearchPathForDirectoriesInDomains(
    ...
    public static func getText(_ path: String, _ fn: @escaping (String?) -> Void) {
    ...
    public static func readText(_ path: String, _ fn: (UTF8Reader?) -> Void) {
        let p = Application.resourcePath(path)
        if FileManager.default.fileExists(atPath: p) {
            let f = FileReader(filename: p)
            let r = UTF8Reader()
    ...
    public static func getJSON(_ path: String, _ fn: @escaping (JSON) -> Void) {
        let p = Application.resourcePath(path)
        if FileManager.default.fileExists(atPath: p) {
            let f = FileReader(filename: p)
            let r = UTF8Reader()
