...
public struct Signal {  // perfect number 0...1
    ...
    public static func realTime(frequency:Double,phase:Double=0,time:Double = ß.time) -> Signal {
        return Signal(Darwin.sin(time*frequency*ß.π*2+phase)*0.5+0.5)
    }
    public static func realTime(period:Double,time:Double = ß.time) -> Signal {
        return Signal(((time*period).truncatingRemainder(dividingBy: 1.0)+1).truncatingRemainder(dividingBy: 1))
    }
    //////////////////////////////////////////////////////////////////////////////////////////////////////////
    //////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public func midPow(_ p:Double) -> Signal {
        if value<0.5 {
            return Signal(Darwin.pow(value*2,p)*0.5)
        } else {
            return Signal((1 - Darwin.pow((1 - (value-0.5)*2) , p)) * 0.5 + 0.5)
        }
    }
    ...
    public func lerp(from:CGFloat,to:CGFloat) -> CGFloat {
        return from + (to-from) * CGFloat(value)
    }
