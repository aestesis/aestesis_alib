...
public struct Float128 {
  ...
  static let bitM: UInt64 = 0x8000_0000_0000_0000
...
func mul128(_ x: UInt64, _ y: UInt64) -> (h: UInt64, l: UInt64) {  // from: http://www.edaboard.com/thread253439.html
  let xl = x & UInt64(0xffff_ffff)
  let xh = x >> UInt64(32)
  let yl = y & UInt64(0xffff_ffff)
  let yh = y >> UInt64(32)
  let xlyl = xl * yl
  let xhyl = xh * yl
  let xlyh = xl * yh
  let xhyh = xh * yh
  var l = xlyl & 0xffff_ffff
  var h = (xlyl >> 32) + (xhyl & 0xffff_ffff) + (xlyh & 0xffff_ffff)
  l += (h & 0xffff_ffff) << 32
  h >>= 32
...
public func + (l: Float128, r: Float128) -> Float128 {
  ...
  if a.n == b.n {
    var e = a.e
    let mr = a.m.addingReportingOverflow(b.m)
    var m = mr.0
    if mr.overflow {
    ...
    let mr = a.m.subtractingReportingOverflow(b.m)
    var m: UInt64 = 0
    var n: Bool = false
    if mr.overflow {
      n = true
    ...
    let mr = b.m.subtractingReportingOverflow(a.m)
    var m: UInt64 = 0
    var n: Bool = false
    if mr.overflow {
      n = true
...
public struct Float256 {
  ...
  public init(_ value: Double) {
    ...
    if value != 0 {
      ...
      m = UInt128(
        upperBits: Float256.bitM + value.significandBitPattern << (UInt64(63 - 52)), lowerBits: 0)
      self.normalize()
    } else {
      n = false
      e = 0
  ...
  public var doubleValue: Double {
    if n {
      return -pow(2.0, Double(e))
        * (Double(m.value.upperBits) * Float256.hp + Double(m.value.lowerBits))
    } else {
      return pow(2.0, Double(e))
        * (Double(m.value.upperBits) * Float256.hp + Double(m.value.lowerBits))
    }
...
public func * (l: Float256, r: Float256) -> Float256 {
  let m = mul256(
    (h: l.m.value.upperBits, l: l.m.value.lowerBits),
    (h: r.m.value.upperBits, l: r.m.value.lowerBits))
  let e = l.e + r.e + Int32(128)
  let n = (l.n != r.n)
  return Float256(mh: m.h, ml: m.l, exponent: e, negative: n)
}

func mul256(_ x: (h: UInt64, l: UInt64), _ y: (h: UInt64, l: UInt64)) -> (
  h: UInt64, l: UInt64, overflow: Bool
