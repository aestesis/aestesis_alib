...
/// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
public class Path: Atom, @unchecked Sendable {
    /// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    /// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    /// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    /// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    public func parse(_ paint: Paint) -> [Contour] {
        ...
        for s in segments {
            let rel =
                (s.cmd.rawValue & PathAbsRel.relative.rawValue) == PathAbsRel.relative.rawValue
            ...
            case PathSegment.cubic_TO:
                ...
                for i in 1...steps {
                    ...
                    let c = 1 - t
                    v.x =
                        ((start.x * c * c * c) + (cp1.x * 3 * t * c * c) + (cp2.x * 3 * t * t * c)
                            + end.x * t * t * t)
                    v.y =
                        ((start.y * c * c * c) + (cp1.y * 3 * t * c * c) + (cp2.y * 3 * t * t * c)
                            + end.y * t * t * t)
                    contour.vertices.append(Contour.Vertice(pos: v))
                    bb = bb.union(Point(v.x, v.y))
                }
            ...
            if let renderer = renderer, renderer.uv == .boundingBox {
                ...
                for c in 0..<points.count {
                    for v in 0..<points[c].vertices.count {
                        points[c].vertices[v].uv =
                            (Point(points[c].vertices[v].position) - bb.origin) * s + o
                    }
                }
            }
            return points
        ...
        for c in points {
            ...
            for i in 0..<c.vertices.count - 1 {
                ...
                cs.vertices.append(
                    Contour.Vertice(
                        pos: v.position + Vec3(x: dv.y, y: -dv.x, z: 0), uv: Point(v.uv.x, 0)))
                inv.vertices.append(
                    Contour.Vertice(
                        pos: v.position + Vec3(x: -dv.y, y: dv.x, z: 0), uv: Point(v.uv.x, 1)))
            }
            let v = c.vertices[c.vertices.count - 1]
            cs.vertices.append(
                Contour.Vertice(pos: v.position + Vec3(x: dv.y, y: -dv.x, z: 0), uv: Point(1, 0)))
            inv.vertices.append(
                Contour.Vertice(pos: v.position + Vec3(x: -dv.y, y: dv.x, z: 0), uv: Point(1, 1)))
            if c.closed {
        ...
        if let renderer = renderer, renderer.uv == .boundingBox {
            ...
            for c in 0..<stroke.count {
                for v in 0..<stroke[c].vertices.count {
                    stroke[c].vertices[v].uv =
                        (Point(stroke[c].vertices[v].position) - bb.origin) * s + o
                }
            }
        }
        return stroke
    ...
    public class Segment {
        ...
        public static func cubicTo(
            _ x0: Double, y0: Double, x1: Double, y1: Double, x2: Double, x3: Double
        ) -> Segment {
    ...
    public class SegmentCubic: Segment {
        ...
        public init(
            _ cmd: PathCommand, _ x0: Double, _ y0: Double, _ x1: Double, _ y1: Double,
    ...
    public struct PathCommand {
        ...
        static var CLOSE_PATH: PathCommand {
            return PathCommand(rawValue: PathSegment.close_PATH.rawValue)
        }
        static var MOVE_TO_ABS: PathCommand {
            return PathCommand(
                rawValue: PathSegment.move_TO.rawValue | PathAbsRel.absolute.rawValue)
        }
        static var MOVE_TO_REL: PathCommand {
            return PathCommand(
                rawValue: PathSegment.move_TO.rawValue | PathAbsRel.relative.rawValue)
        }
        static var LINE_TO_ABS: PathCommand {
            return PathCommand(
                rawValue: PathSegment.line_TO.rawValue | PathAbsRel.absolute.rawValue)
        }
        static var LINE_TO_REL: PathCommand {
            return PathCommand(
                rawValue: PathSegment.line_TO.rawValue | PathAbsRel.relative.rawValue)
        }
        static var HLINE_TO_ABS: PathCommand {
            return PathCommand(
                rawValue: PathSegment.hline_TO.rawValue | PathAbsRel.absolute.rawValue)
        }
        static var HLINE_TO_REL: PathCommand {
            return PathCommand(
                rawValue: PathSegment.hline_TO.rawValue | PathAbsRel.relative.rawValue)
        }
        static var VLINE_TO_ABS: PathCommand {
            return PathCommand(
                rawValue: PathSegment.vline_TO.rawValue | PathAbsRel.absolute.rawValue)
        }
        static var VLINE_TO_REL: PathCommand {
            return PathCommand(
                rawValue: PathSegment.vline_TO.rawValue | PathAbsRel.relative.rawValue)
        }
        static var QUAD_TO_ABS: PathCommand {
            return PathCommand(
                rawValue: PathSegment.quad_TO.rawValue | PathAbsRel.absolute.rawValue)
        }
        static var QUAD_TO_REL: PathCommand {
            return PathCommand(
                rawValue: PathSegment.quad_TO.rawValue | PathAbsRel.relative.rawValue)
        }
        static var CUBIC_TO_ABS: PathCommand {
            return PathCommand(
                rawValue: PathSegment.cubic_TO.rawValue | PathAbsRel.absolute.rawValue)
        }
        static var CUBIC_TO_REL: PathCommand {
            return PathCommand(
                rawValue: PathSegment.cubic_TO.rawValue | PathAbsRel.relative.rawValue)
        }
        static var SQUAD_TO_ABS: PathCommand {
            return PathCommand(
                rawValue: PathSegment.squad_TO.rawValue | PathAbsRel.absolute.rawValue)
        }
        static var SQUAD_TO_REL: PathCommand {
            return PathCommand(
                rawValue: PathSegment.squad_TO.rawValue | PathAbsRel.relative.rawValue)
        }
        static var SCUBIC_TO_ABS: PathCommand {
            return PathCommand(
                rawValue: PathSegment.scubic_TO.rawValue | PathAbsRel.absolute.rawValue)
        }
        static var SCUBIC_TO_REL: PathCommand {
            return PathCommand(
                rawValue: PathSegment.scubic_TO.rawValue | PathAbsRel.relative.rawValue)
        }
        static var SCCWARC_TO_ABS: PathCommand {
            return PathCommand(
                rawValue: PathSegment.sccwarc_TO.rawValue | PathAbsRel.absolute.rawValue)
        }
        static var SCCWARC_TO_REL: PathCommand {
            return PathCommand(
                rawValue: PathSegment.sccwarc_TO.rawValue | PathAbsRel.relative.rawValue)
        }
        static var SCWARC_TO_ABS: PathCommand {
            return PathCommand(
                rawValue: PathSegment.scwarc_TO.rawValue | PathAbsRel.absolute.rawValue)
        }
        static var SCWARC_TO_REL: PathCommand {
            return PathCommand(
                rawValue: PathSegment.scwarc_TO.rawValue | PathAbsRel.relative.rawValue)
        }
        static var LCCWARC_TO_ABS: PathCommand {
            return PathCommand(
                rawValue: PathSegment.lccwarc_TO.rawValue | PathAbsRel.absolute.rawValue)
        }
        static var LCCWARC_TO_REL: PathCommand {
            return PathCommand(
                rawValue: PathSegment.lccwarc_TO.rawValue | PathAbsRel.relative.rawValue)
        }
        static var LCWARC_TO_ABS: PathCommand {
            return PathCommand(
                rawValue: PathSegment.lcwarc_TO.rawValue | PathAbsRel.absolute.rawValue)
        }
        static var LCWARC_TO_REL: PathCommand {
            return PathCommand(
                rawValue: PathSegment.lcwarc_TO.rawValue | PathAbsRel.relative.rawValue)
        }
    }
    /*
