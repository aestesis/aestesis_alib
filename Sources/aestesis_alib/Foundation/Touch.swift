...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////////////////
public class TouchManager: NodeUI, @unchecked Sendable {
    //////////////////////////////////////////////////////////////////////////////////////////////////////////
    //////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public func touches(_ touches: [TouchLocation]) -> Bool {
                ...
                click = self.wait(0.8) {
                    ...
                    if !self.pressed {
                        if (self.pos - self.mpos).length <= self.distanceCancelClick {
                            Debug.info(
                                "## \(self.view.className) ## dispatch click, pos:\(self.pos)")
        ...
        case .released, .cancelled:
            ...
            if acceptMove {
                ...
                var pm = pos
                if magnet.width != 0 {
                    if abs(vAccel.x) > 0.5 && ß.sign(pos.x - mpos.x) == ß.sign(vAccel.x)
                        && abs(vAccel.x) > abs(vAccel.y)
                    {
                        let e0 =
                            round(
                                ((mpos.x - magnetOffset.x) / magnet.w) + ß.sign(vAccel.x) * page.w)
                            * magnet.w + magnetOffset.x
                        pm.x = bounds.constrains(point: Point(e0, 0)).x
                    } else {
                        let e0 =
                            round((pos.x - magnetOffset.x) / magnet.w) * magnet.w + magnetOffset.x
                        pm.x = bounds.constrains(point: Point(e0, 0)).x
                    }
                }
                if magnet.height != 0 {
                    if abs(vAccel.y) > 0.5 && ß.sign(pos.y - mpos.y) == ß.sign(vAccel.y)
                        && abs(vAccel.y) > abs(vAccel.x)
                    {
                        let e0 =
                            round(
                                ((mpos.y - magnetOffset.y) / magnet.h) + ß.sign(vAccel.y) * page.h)
                            * magnet.h + magnetOffset.y
                        pm.y = bounds.constrains(point: Point(0, e0)).y
                    } else {
                        let e0 =
                            round((pos.y - magnetOffset.y) / magnet.h) * magnet.h + magnetOffset.y
                        pm.y = bounds.constrains(point: Point(0, e0)).y
                    }
                }
                if let magnets = magnetsH, magnets.count > 0 {
            ...
            if acceptMove && mpos == pos && (magnet.width != 0 || magnet.height != 0) {
                ...
                if magnet.width != 0 {
                    ...
                    if abs(d.x) > (0.05 * magnet.w) && abs(d.x) > abs(d.y) {
                        stop.x =
                            round(((start.x - magnetOffset.x) + ß.sign(d.x) * magnet.w) / magnet.w)
                            * magnet.w + magnetOffset.x
                    } else {
                        stop.x = start.x
                    }
                ...
                if magnet.height != 0 {
                    ...
                    if abs(d.y) > (0.05 * magnet.h) && abs(d.y) > abs(d.x) {
                        stop.y =
                            round(((start.y - magnetOffset.y) + ß.sign(d.y) * magnet.h) / magnet.h)
                            * magnet.h + magnetOffset.y
                    } else {
                        stop.y = start.y
                    }
    ...
    public func magnetSwipe(_ p: Point) -> Bool {
        ...
        if magnet.w > 0 && abs(p.x) > 0 {
            let oe =
                (round(((pos.x - magnetOffset.x) / magnet.w) + ß.sign(p.x) * page.w) * magnet.w)
                + magnetOffset.x
        ...
        if magnet.h > 0 && abs(p.y) > 0 {
            let oe =
                (round(((pos.y - magnetOffset.y) / magnet.h) + ß.sign(p.y) * page.h) * magnet.h)
                + magnetOffset.y
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////////////
    public init(
        view: View, coef: Double? = nil, accel: Double = 1, distanceCancelClick: Double? = nil,
        distanceAcceptMove: Double = 0
    ) {
        self.accel = accel
    ...
    override public func detach() {
        ...
        onClick.removeAll()
        onDblClick.removeAll()
        onVirtualMoveBegun.removeAll()
        onVirtualMoved.removeAll()
        onVirtualMoveEnded.removeAll()
        super.detach()
    }
    //////////////////////////////////////////////////////////////////////////////////////////////////////////
    //////////////////////////////////////////////////////////////////////////////////////////////////////////
...
public struct Keybutton: Key {
    ...
    public enum Name {
        ...
        public var isArrow: Bool {
            return (self == .arrowUp) || (self == .arrowDown) || (self == .arrowLeft)
                || (self == .arrowRight)
        }
        public func nearEqual(_ b: Name) -> Bool {
            ...
            if a == b {
                return true
            } else if (a == .buttonA || a == .buttonTop || a == .buttonBottom || a == .buttonLeft
                || a == .buttonRight)
                && (b == .buttonA || b == .buttonTop || b == .buttonBottom || b == .buttonLeft
                    || b == .buttonRight)
            {
                return true
            }
...
public struct MouseOver: CustomStringConvertible {
    ...
    init(
        state: State = State.invalid, position: Point = Point.zero, delta: Point = Point.zero,
        buttons: Button = .none
    ) {
        self.state = state
        self.position = position
...
public struct TouchLocation {
    ...
    static func transform(touches: [TouchLocation], matrix: Mat4) -> [TouchLocation] {
        ...
        for t in touches {
            ...
            tr.append(
                TouchLocation(state: t.state, position: Point(v.x, v.y), pressure: t.pressure))
        }
        return tr
    }
}
