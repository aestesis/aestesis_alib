...
public class LUT: JsonConvertible {
    // HSV generate in Texture3D.fill(hsv:Vec3)
        ...
        do {
            ...
            for li in data.split("\n") {
                ...
                if l.length > 0 && l[0] != "#" {
                    ...
                    } else if w.count > 2, let r = Double(w[0]), let g = Double(w[1]),
                        let b = Double(w[2])
                    {
    ...
    public func createTexture2D(parent: NodeUI) -> Texture2D {
        let b = Texture2D(
            parent: parent, size: Size(Double(self.size), Double(self.size * self.size)))
