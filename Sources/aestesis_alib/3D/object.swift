...
open class Object : Node3D {
    ...
    public init(parent:NodeUI,matrix:Mat4=Mat4.identity,mesh:Mesh,materials:[String:Material]) {
        self.mesh = mesh
    ...
    public func updateBuffers() {
        ...
        if bufferVertices == nil {
            bufferVertices = self.persitentBuffer(MemoryLayout<GPUvertice>.stride*mesh.vertices.count)
        }
        ...
        for m in materials.values {
            ...
            ptr[0] = GPUmaterial(ambient:m.ambient.infloat4,diffuse:m.diffuse.infloat4,specular:m.specular.infloat4,shininess:Float32(m.shininess))
        }
    ...
    func render(to g:Graphics,world:Mat4,material:Material) {
        guard let renderer = renderer, let camera = renderer.camera, let f=mesh.faces[material.name], f.count>0, let bv=bufferVertices, let bf=bufferFaces[material.name], let bm=bufferMaterials[material.name] else { return }
        let prog = "program.3d.\(renderer.lightsProgram)"
        if let ktexture = material.texture, let texture = self[ktexture] as? Texture2D, texture.ready {
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////////////////
open class ObjectCollection : Object {
