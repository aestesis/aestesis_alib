...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////////////////
open class ObjectOld: Node3D, @unchecked Sendable {
    ...
    public init(parent: Node3D, cylinder: Cylinder, material: String = "material.default") {
        ...
        self.meshMatrix =
            Mat4.scale(Vec3(x: cylinder.radius, y: cylinder.direction.length, z: cylinder.radius))
            * Mat4.rotation(axis: cylinder.direction, angle: 0)
        super.init(parent: parent, matrix: Mat4.translation(cylinder.center))
        if self[mesh] is MeshOld {
            dispatchInit()
        } else if let db = self.db {
            db[mesh] = MeshOld(
                parent: db, cylinder: Cylinder(radius: cylinder.radius / cylinder.direction.length),
                factor: 16)
            dispatchInit()
        }
        if material != "material.default" {
    ...
    public init(
        parent: Node3D, box: Box, material: String = "material.default", inversNormals: Bool = false
    ) {
        self.mesh = "mesh.box"
        ...
        if self[mesh] is MeshOld {
            ...
            db[mesh] = MeshOld(
                parent: db, box: Box(x: -0.5, y: -0.5, z: -0.5, w: 1, h: 1, d: 1),
                inversNormals: inversNormals)
            dispatchInit()
        }
        if material != "material.default" {
            self["material.default"] = material
