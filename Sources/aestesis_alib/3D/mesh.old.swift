...
open class MeshOld: NodeRenderer, @unchecked Sendable {
                    ...
                    let decodeVertice: ((String) -> (Int)) = { s in
                        ...
                        case 3:
                            if let iv = Int(e[0]), let iuv = Int(e[1]), let inorm = Int(e[2]) {
                                let v = Vertex(
                                    position: lvert[iv - 1], normal: lnorm[inorm - 1],
                                    uv: luv[iuv - 1])
                                if let i = self.index(vertice: v) {
                                    return i
                                } else {
                            ...
                            if t.length > 0 && t[0] != "#" {
                                ...
                                if cmd == "v" {
                                    ...
                                    let p =
                                        ...
                                        + mtlfile[mtlfile.lastIndexOf("/")! + 1..<mtlfile.length]
                                                ...
                                                if cmd == "newmtl" {
                                                    ...
                                                    if cmd == "Ka" {
                                                        cmat!.ambient = Color(
                                                            ...
                                                            rgb: Color(
                                                                a: 1, r: Double(p[1])!,
                                                                g: Double(p[2])!, b: Double(p[3])!))
                                                    } else if cmd == "Kd" {
                                                        cmat!.diffuse = Color(
                                                            a: 1, r: Double(p[1])!,
                                                            g: Double(p[2])!, b: Double(p[3])!)
                                                        cmat!.ambient =
                                                            cmat!.diffuse * cmat!.ambient * 0.4
            ...
            for j in 0...factor {
                ...
                for i in 0..<factor {
                    ...
                    self.vertices.append(
                        Vertex(
                            position: p, normal: p.normalized,
                            uv: Point(x: Double(i) / Double(factor), y: Double(j) / Double(factor)))
                    )
                }
            }
    ...
    public init(parent: NodeUI, cylinder: Cylinder, factor: Int = 20) {
        ...
        self.zz {
            ...
            self.vertices.append(
                Vertex(position: cylinder.center - cylinder.direction * 0.5, normal: Vec3(y: -1)))
    ...
    public init(parent: NodeUI, box: Box, inversNormals: Bool) {
        ...
        self.zz {
            let mat = "material.default"
            self.faces[mat] = [Int32]()
            let addfaces: ((Int, Int, Int, Int, Bool) -> Void) = { v0, v1, v2, v3, invers in
                if !invers {
            ...
            for x in 0...1 {
                ...
                let n =
                    inversNormals
                    ? Vec3(x: Double(-dx), y: 0, z: 0) : Vec3(x: Double(dx), y: 0, z: 0)
                let v0 = self.vertices.appendIndex(
                    Vertex(position: box.point(Double(x), 0, 0), normal: n, color: .white))
                let v1 = self.vertices.appendIndex(
                    Vertex(position: box.point(Double(x), 1, 0), normal: n, color: .white))
                let v2 = self.vertices.appendIndex(
                    Vertex(position: box.point(Double(x), 1, 1), normal: n, color: .white))
                let v3 = self.vertices.appendIndex(
                    Vertex(position: box.point(Double(x), 0, 1), normal: n, color: .white))
            ...
            for y in 0...1 {
                ...
                let n =
                    inversNormals
                    ? Vec3(x: 0, y: Double(-dy), z: 0) : Vec3(x: 0, y: Double(dy), z: 0)
                let v0 = self.vertices.appendIndex(
                    Vertex(position: box.point(0, Double(y), 0), normal: n, color: .white))
                let v1 = self.vertices.appendIndex(
                    Vertex(position: box.point(1, Double(y), 0), normal: n, color: .white))
                let v2 = self.vertices.appendIndex(
                    Vertex(position: box.point(1, Double(y), 1), normal: n, color: .white))
                let v3 = self.vertices.appendIndex(
                    Vertex(position: box.point(0, Double(y), 1), normal: n, color: .white))
            ...
            for z in 0...1 {
                ...
                let n =
                    inversNormals
                    ? Vec3(x: 0, y: 0, z: Double(-dz)) : Vec3(x: 0, y: 0, z: Double(dz))
                let v0 = self.vertices.appendIndex(
                    Vertex(position: box.point(0, 0, Double(z)), normal: n, color: .white))
                let v1 = self.vertices.appendIndex(
                    Vertex(position: box.point(1, 0, Double(z)), normal: n, color: .white))
                let v2 = self.vertices.appendIndex(
                    Vertex(position: box.point(1, 1, Double(z)), normal: n, color: .white))
                let v3 = self.vertices.appendIndex(
                    Vertex(position: box.point(0, 1, Double(z)), normal: n, color: .white))
    ...
    public func computeNormals() {
        ...
        for i in lfaces.keys {
            ...
            if let fl = lfaces[i] {
                ...
                for f in fl {
                    n =
                        n + (vertices[Int(f.v1)].position - v0)
                        ^ (vertices[Int(f.v2)].position - v0).normalized
                }
        ...
        if needsVerticesUpdate {
            ...
            if bufferVertices == nil {
                bufferVertices = self.persitentBuffer(
                    MemoryLayout<GPUvertice>.stride * vertices.count)
            }
            ...
            for m in faces.keys {
                if let f = faces[m] {
                    if bufferFaces[m] == nil {
                        bufferFaces[m] = self.persitentBuffer(
                            MemoryLayout<Float32>.stride * f.count)
                    }
    ...
    public func render(to g: Graphics, world: Mat4, library: NodeUI, opaque: Bool) -> Bool {
        ...
        for kmat in faces.keys {
            ...
            if let mat = library[material] as? MaterialOld {
                if mat.transparent != opaque, let f = faces[kmat], let bv = bufferVertices,
                    let bf = bufferFaces[kmat], f.count > 0
                {
