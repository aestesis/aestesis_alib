...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  class ARDelegate: NSObject, ARSessionDelegate {
    ...
    let frame: (ARFrame) -> Void
    let anchors: (AnchorCommand, [ARAnchor]) -> Void
    public init(
      frame: @escaping (ARFrame) -> Void, anchors: @escaping (AnchorCommand, [ARAnchor]) -> Void
    ) {
      self.frame = frame
        ...
        frame: { frame in
          ...
          if let tc = self.texcache {
            ...
            if CVMetalTextureCacheCreateTextureFromImage(
              nil, tc, frame.capturedImage, nil, .bgra8Unorm, w, h, 0, &t) == kCVReturnSuccess,
              let tt = t, let mt = CVMetalTextureGetTexture(tt)
            {
              self.ui {
                if let o = self.image {
                  o.detach()
                ...
                self.image = Bitmap(
                  parent: self,
                  texture: Texture2D(parent: self, size: Size(Double(w), Double(h)), texture: mt))
              }
            }
            ...
            self.camera.euler = Euler(
              pitch: Double(cam.eulerAngles[1]), roll: Double(cam.eulerAngles[0]),
              yaw: Double(cam.eulerAngles[3]))
