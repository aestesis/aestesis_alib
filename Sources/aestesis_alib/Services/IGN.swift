...
class IGN {
    ...
    public static func search(query:String,fn:@escaping ((Any?)->())) {
        ...
        let attributes = ["xmlns:xsi":"http://www.w3.org/2001/XMLSchema-instance","xmlns:xls":"http://www.opengis.net/xls","xmlns:gml":"http://www.opengis.net/gml","xmlns":"http://www.opengis.net/xls","xsi:schemaLocation":"http://www.opengis.net/xls http://schemas.opengis.net/ols/1.2/olsAll.xsd","version":"1.2"]
        let xls = xreq.addChild(name:"XLS",attributes:attributes)
        xls.addChild(name:"RequestHeader")
        let req = xls.addChild(name:"Request",attributes:["requestID":"1","version":"1.2","methodName":"GeocodeRequest","maximumResponses":"1"])
    ...
    public static func geo(address:String,fn:@escaping (GeoPoint?)->()) {
        self.search(query: address) { r in
            if let xdoc = r as? AEXMLDocument {
                if let t = xdoc.root["Response"]["GeocodeResponse"]["GeocodeResponseList"]["GeocodedAddress"]["gml:Point"]["gml:pos"].value {
                    let tc = t.split(" ")
