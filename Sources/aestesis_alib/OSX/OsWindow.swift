...
public class OsWindow: NSWindow {
    ...
    public init(frame: NSRect, title: String, splash: String? = nil) {
        ...
        super.init(
            contentRect: frame,
            styleMask: [.resizable, .closable, .miniaturizable, .titled, .unifiedTitleAndToolbar],
        ...
        if let filename = splash {
            let s = Splash(
                frame: CGRect(x: 0, y: 0, width: frame.width, height: frame.height), asset: filename)
            self.view!.addSubview(s)
            var waitSplash: (() -> Void)? = nil
            waitSplash = {
    ...
    public var isFullScreen: Bool {
        return (styleMask.rawValue & NSWindow.StyleMask.fullScreen.rawValue)
...
class Splash: NSView {
    ...
    override func draw(_ dirtyRect: NSRect) {
        if let img = image {
            let s = Double(
                max(self.bounds.width / img.size.width, self.bounds.height / img.size.height))
...
class OsWindowDelegate: NSObject, NSWindowDelegate {
    ...
    @objc func windowWillResize(_ sender: NSWindow, to frameSize: NSSize) -> NSSize {
        if let window = sender as? OsWindow, let ratio = window.ratio {
            let d = sqrt(frameSize.width * frameSize.width + frameSize.height * frameSize.height)
            let sz = Size(diagonal:d,ratio:ratio)
            return sz.system
        }
        return CGSize(
            width: max(frameSize.width, minSize.width), height: max(frameSize.height, minSize.height))
    }
    ...
    @objc func window(
        ...
        return NSApplication.PresentationOptions(
            rawValue: proposedOptions.rawValue
            | NSApplication.PresentationOptions.autoHideToolbar.rawValue)
    }
...
@objc open class OsAppDelegate: NSObject, NSApplicationDelegate {
    ...
    var callback: (OsAppDelegate) -> Void
    public init(_ callback: @escaping (OsAppDelegate) -> Void) {
