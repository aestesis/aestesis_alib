...
public class OsView: MTKView, SystemView {
    ...
    override public func viewDidMoveToSuperview() {
        if superview == nil {
            ...
            let ta = NSTrackingArea(
                ...
                options: [
                    NSTrackingArea.Options.activeInKeyWindow, NSTrackingArea.Options.mouseEnteredAndExited,
    ...
    func memoMouseOver(_ evt: NSEvent, state: MouseOver.State, buttons: MouseOver.Button = .none) {
        ...
        lock.sync {
            self.mMouseOver.enqueue(
                MouseOver(
                    state: state, position: p, delta: Point(Double(evt.deltaX), Double(evt.deltaY)),
                    buttons: buttons))
        }
    }
    //////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public override func keyUp(with theEvent: NSEvent) {
        let k = Keyboard(
            device: 0, text: theEvent.characters!, keyCode: Int(theEvent.keyCode), pressed: false)
        lock.sync {
            self.mKeys.enqueue(k)
        }
    }
        ...
        if let v = view as? OsView, let device = view.device {
            if let w = v.window, !w.isMiniaturized && w.isOnActiveSpace {
                if !v.destroy && v.viewport == nil {
                    let sc:Double = Double(w.screen?.backingScaleFactor ?? CGFloat(Device.screenScale))
                    let scale = Size(sc,sc)
                    view.depthStencilPixelFormat = .depth32Float
                    v.viewport = Viewport(
                        systemView: v, device: device, size: Size(v.bounds.size), scale: scale, threads: threads)
