...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////////////
    //////////////////////////////////////////////////////////////////////////////////////////////////////////
    @MainActor public class OsView: MTKView, @MainActor SystemView {
        //////////////////////////////////////////////////////////////////////////////////////////////////////////
        //////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        override public func viewDidMoveToSuperview() {
            if superview == nil {
                ...
                let ta = NSTrackingArea(
                    rect: CGRect(
                        x: CGFloat.leastNormalMagnitude, y: CGFloat.leastNormalMagnitude,
        ...
        override public func viewDidChangeBackingProperties() {
            if let w = window, let vp = viewport {
                vp.onBackingScale.dispatch(Size(Double(w.backingScaleFactor)))
            }
        }
        //////////////////////////////////////////////////////////////////////////////////////////////////////////
        //////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        func memoMouseOver(
            _ evt: NSEvent, state: MouseOver.State, buttons: MouseOver.Button = .none
        ) {
            var p = Point(evt.locationInWindow)
            p.y = Double(self.bounds.height) - p.y
            mousePos = p
            lock.sync {
                self.mMouseOver.enqueue(
                    MouseOver(
                        state: state, position: p,
                        delta: Point(Double(evt.deltaX), Double(evt.deltaY)),
                        buttons: buttons))
            }
        }
        //////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        public override func keyDown(with theEvent: NSEvent) {
            let k = Keyboard(
                device: 0, text: theEvent.characters!, keyCode: Int(theEvent.keyCode), pressed: true
            )
            lock.sync {
        ...
        public override func keyUp(with theEvent: NSEvent) {
            let k = Keyboard(
                device: 0, text: theEvent.characters!, keyCode: Int(theEvent.keyCode),
                pressed: false)
            lock.sync {
                self.mKeys.enqueue(k)
            }
    ...
    class OsViewDelegate: NSViewController, MTKViewDelegate {
        ...
        func draw(in view: MTKView) {
            if let v = view as? OsView, let device = view.device {
                if let w = v.window, !w.isMiniaturized && w.isOnActiveSpace {
                    if !v.destroy && v.viewport == nil {
                        let sc: Double = Double(
                            w.screen?.backingScaleFactor ?? CGFloat(Device.screenScale))
                        let scale = Size(sc, sc)
                        view.depthStencilPixelFormat = .depth32Float
                        v.viewport = Viewport(
                            systemView: v, device: device, size: Size(v.bounds.size), scale: scale,
                            threads: threads)
                        if let w = v.window as? OsWindow {
                            w.onStartUI.dispatch(v.viewport!)
                            if let vp = v.viewport {
                                vp.onBackingScale.dispatch(scale)
                            }
                        } else {  // flutter view
                            v.onStartUI.dispatch(v.viewport!)
                            if let vp = v.viewport {
                                vp.onBackingScale.dispatch(scale)
                            }
                        }
                    ...
                    if let vp = v.viewport {
                        ...
                        vp.update()
                        if let desc = v.currentRenderPassDescriptor,
