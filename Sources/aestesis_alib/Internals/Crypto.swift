...
public class Crypto {
    public static func hmacSHA256(string:String,key:String) -> String {
    ...
    public static func hmacSHA1(string:String,key:String) -> String {
        let stringData = string.data(using:.utf8)!
        let keyData = key.data(using:.utf8)!
        let stringBytes = [UInt8](stringData)
        let keyBytes = [UInt8](keyData)
        let hash = NSMutableData(length:Int(CC_SHA1_DIGEST_LENGTH))!
        CCHmac(UInt32(kCCHmacAlgSHA1), keyBytes, keyBytes.count, stringBytes, stringBytes.count, hash.mutableBytes)
        return hash.base64EncodedString(options:NSData.Base64EncodingOptions(rawValue:0))
    }
    public static func hmacSHA1(string:String,hexKey:String) -> String {    // hexKey ~ "a0bF00"
        let stringData = string.data(using:.utf8)!
        let keyData = hexKey.dataFromHex()!
        let stringBytes = [UInt8](stringData)
        let keyBytes = [UInt8](keyData)
        let hash = NSMutableData(length:Int(CC_SHA1_DIGEST_LENGTH))!
        CCHmac(UInt32(kCCHmacAlgSHA1), keyBytes, keyBytes.count, stringBytes, stringBytes.count, hash.mutableBytes)
        return (hash as Data).hexEncodedString()
    }
