...
public class AudioOutput: Atom, @unchecked Sendable {
    ...
    public init(nchan: Int, sampleRate: Int, fill: @escaping FillCallback) {
        ...
        format = AudioStreamBasicDescription(
            ...
            mBytesPerPacket: UInt32(bytesPerPacket), mFramesPerPacket: 1,
            mBytesPerFrame: UInt32(bytesPerPacket), mChannelsPerFrame: UInt32(nchan),
            mBitsPerChannel: UInt32(bytesPerChannel * 8), mReserved: 0)
        super.init()
        ...
        let _ = Thread({
            ...
            AudioQueueNewOutput(
                &self.format, callback, this, CFRunLoopGetCurrent(),
                CFRunLoopMode.commonModes.rawValue, 0, &output)
    ...
    func enqueue(_ queue: AudioQueueRef, _ buffer: AudioQueueBufferRef) {
        if !release {
            ...
            let sz =
                fill(
                    data,
                    Int(buffer.pointee.mAudioDataBytesCapacity) / Int(self.format.mBytesPerFrame))
                * Int(self.format.mBytesPerFrame)
