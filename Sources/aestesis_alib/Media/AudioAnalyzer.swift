...
public class AudioAnalyzer : Atom {
    ...
    public func feed(_ buffer:[Float],offset:Int = 0,count:Int = 0) {
        ...
        @Sendable var b:Float = 0
        @Sendable var m:Float = 0
        @Sendable var t:Float = 0
        var samples = self.samples
        var bass = self.bass
        var medium = self.medium
        var treeble = self.treeble
        var ma:Float=0
        ...
        let fft = fftProcessor.transform(samples: samples) ?? FFT.Result(count: AudioAnalyzer.samplesCount / 2)
        var enveloppe : Double = 0
...
public class FFT {
    ...
    init(count:Int) {
        do {
            dsp = try vDSP.DiscreteFourierTransform(count: count, direction: .forward, transformType: .complexComplex, ofType: Float.self)
