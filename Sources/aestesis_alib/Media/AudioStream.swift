            ...
            self.ice=Web.getICE(url:daurl,meta:{ m in
                                        ...
                                        if let b=decode.read(sz) {
                                            if let fn=mono {
                                                ...
                                                b.withUnsafeBytes { bytes in
                                                    let bb =  bytes.baseAddress!.assumingMemoryBound(to: Float.self)
                                                    if nchan==1 {
                                                        for j in 0..<zb.count {
                                                            zb[j] = bb[j]
                                                        }
                                                    ...
                                                    memcpy(samples, bytes.baseAddress!, bytes.count)
                                                }
                                                fn(zb)
                                            }
                                            ploutch = 0
                ...
                if let fn=error {
                    if let erres:Response.ResponseError = err.get() {
                        if erres.response.status >= 302 && erres.response.status <= 307 {
                            if let loc=erres.response.header["Location"] {  // redirection
                                if loc.contains("http:") {
                                    daurl=loc
                                    doit!()
                                } else {
                                    daurl += loc    // TODO: not sure ????
                                    doit!()
                                }
                            }
                        } else {
...
public class AudioConverter : Stream {
    ...
    public func set(cookie:[UInt8]) {
        ...
        _ = cookie.withUnsafeBufferPointer { ptr in
            AudioConverterSetProperty(cv!, kAudioConverterCompressionMagicCookie,sz,ptr.baseAddress!)
        }
    }
            ...
            let cbinput:AudioConverterComplexInputDataProc={ (cfref,npackets,bufList,desc,user) in
                ...
                this.inlock.synced {
                    if let ac=this.incontrols.dequeue() {
                        ...
                        desc! [0] = this.pd
                        npackets [0] = UInt32(min(ac.desc.count,this.pdcount))
                        if ac.desc.count>this.pdcount {
                            Debug.error("too much descs")
                        }
                    } else {
        ...
        lock.synced {
            if let sid = self.sid {
                ...
                data.withUnsafeBufferPointer { ptr in
                    let st = AudioFileStreamParseBytes(sid, UInt32(count), ptr.baseAddress!.advanced(by: offset), AudioFileStreamParseFlags(rawValue:0))
                    if st == 0 {
                        dc = data.count
                    } else {
